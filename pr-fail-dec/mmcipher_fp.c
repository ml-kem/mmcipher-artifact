//  mmcipher_fp.c
//  === mmCipher Failure Probability estimate

#include <stdio.h>
#include <math.h>
#include "zp_dist.h"

//  mmCipher Theorem D.6. (Correctness)

double mmcipher_fp( int64_t     prm_n,
                    int64_t     prm_m,
                    int64_t     prm_bar_nu,
                    int64_t     prm_d_u,
                    int64_t     prm_d_v,
                    ...
                    size_t      eps)
{
    ...
    printf("\n=== mmcipher_fp( %ld, %ld, %ld, %ld, %ld, %s, %s, %s, %zu )\n",
...
double mmcipher_fp_summary(size_t eps)
{
    ...
    //  mmCipherKEM
    fp_128 = dn * mmcipher_fp(4, 4, 3, 10, 0, "15.90", "368459.34", true, eps);
    fp_192 = dn * mmcipher_fp(7, 7, 2, 11, 0, "15.90", "488797.36", true, eps);
    fp_256 = dn * mmcipher_fp(9, 9, 2, 11, 0, "15.90", "554941.07", true, eps);

    printf("\n=== mmCipherKEM Summary N= %ld\n", n);
    printf("mmCipherKEM-128: %e  2^%g\n", fp_128, log2(fp_128));
    printf("mmCipherKEM-192: %e  2^%g\n", fp_192, log2(fp_192));
    printf("mmCipherKEM-256: %e  2^%g\n", fp_256, log2(fp_256));


    //  mmCipherPKE
    fp_128 = dn * mmcipher_fp(4, 4, 3, 10, 2, "15.90", "368459.34", false, eps);
    fp_192 = dn * mmcipher_fp(7, 7, 2, 11, 2, "15.90", "488797.36", false, eps);
    fp_256 = dn * mmcipher_fp(9, 9, 2, 11, 2, "15.90", "554941.07", false, eps);

    printf("\n=== mmCipherPKE Summary N= %ld\n", n);
    printf("mmCipherPKE-128: %e  2^%g\n", fp_128, log2(fp_128));
    printf("mmCipherPKE-192: %e  2^%g\n", fp_192, log2(fp_192));
    printf("mmCipherPKE-256: %e  2^%g\n", fp_256, log2(fp_256));
